...
public class Solution {
	int findLargest // Finds the largest element in a positive array.
	(int[][] array)
	...
	int[][] transpose // Transposes a double[][] array.
	(int[][] array) {
	...
	int[][] copyOf // Copies all elements of an array to a new array.
	(int[][] original) {
	...
	int hgAlgorithm(int[][] harmonyScores, String sumType) {
		int[][] cost = copyOf(harmonyScores); // Create the cost matrix
		...
		int[][] assignment = new int[harmonyScores.length][2]; // Create the returned array.
		...
		for (int i = 0; i < assignment.length; i++) {
			sum = sum + harmonyScores[assignment[i][0]][assignment[i][1]];
		}
		return sum;
	}

	int hg_step1(int step, int[][] cost) {
		// What STEP 1 does:
	...
	int hg_step2(int step, int[][] cost, int[][] mask, int[] rowCover, int[] colCover) {
		// What STEP 2 does:
	...
	int hg_step3(int step, int[][] mask, int[] colCover) {
		// What STEP 3 does:
	...
	int hg_step4(int step, int[][] cost, int[][] mask, int[] rowCover, int[] colCover, int[] zero_RC) {
		// What STEP 4 does:
	...
	int[] findUncoveredZero // Aux 1 for hg_step4.
	...
	int hg_step5(int step, int[][] mask, int[] rowCover, int[] colCover, int[] zero_RC) {
		// What STEP 5 does:
	...
	int findStarInCol // Aux 1 for hg_step5.
	(int[][] mask, int col) {
	...
	int findPrimeInRow // Aux 2 for hg_step5.
	(int[][] mask, int row) {
		int c = -1;
	...
	void convertPath // Aux 3 for hg_step5.
	...
	void erasePrimes // Aux 4 for hg_step5.
	(int[][] mask) {
		for (int i = 0; i < mask.length; i++) {
			for (int j = 0; j < mask[i].length; j++) {
				if (mask[i][j] == 2) {
	...
	void clearCovers // Aux 5 for hg_step5 (and not only).
	...
	int hg_step6(int step, int[][] cost, int[] rowCover, int[] colCover, int maxCost) {
		// What STEP 6 does:
	...
	int findSmallest // Aux 1 for hg_step6.
	...
	int maximumHarmonyScore(int[][] harmonyScores) {
		int res = hgAlgorithm(harmonyScores, "max");
		return res;
